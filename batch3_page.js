/* ============================================================
   batch3_page.js  —  Vendra v4, Batch 3 of 3
   Drop in your project root (C:\Users\kvngs\VENDRA), then run:
       node batch3_page.js
   ------------------------------------------------------------
   Replaces app/page.tsx with the v4 homepage:
     - <Nav theme="v4" /> (light skin from Batch 2)
     - photo hero + italic-gold headline
     - "List it. Sell it." beat + wallet phone mock
     - marketplace grid using /showcase images (graceful gradient fallback)
     - on-chain trust trio + particle-globe CTA + footer
     - keeps your handleGetStarted routing logic intact
   Backs up your current homepage to app/page.tsx.v3bak (once).
   ============================================================ */
const fs = require('fs');
...
if (!fs.existsSync(path.join(root, 'app'))) {
  console.error('\n  x  app/ not found. Run this from your project root (folder with app/, package.json).\n');
  process.exit(1);
}
if (fs.existsSync(pagePath) && !fs.existsSync(pagePath + '.v3bak')) {
  fs.copyFileSync(pagePath, pagePath + '.v3bak');
  console.log('  .  backup written  -> app/page.tsx.v3bak');
}
...
console.log('  +  app/page.tsx rewritten as the v4 homepage');
console.log('\n  Batch 3 done. Make sure your 9 images are in public/showcase/ (they are).');
console.log('  Next: npm run build  (should pass), then npm run dev to view, then git push.\n');
