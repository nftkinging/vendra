/* ============================================================
   batch10_homepage.js  —  Vendra v4, Batch 10 (homepage -> live DB)
   Drop in project root (C:\Users\kvngs\VENDRA), then run:
       node batch10_homepage.js
   ------------------------------------------------------------
   Same v4 homepage, but the product grid now reads LIVE stores
   (one featured product per store, up to 8) and each card links to
   its product page /store/[slug]/[id]. If the DB is empty it falls
   back to the original hardcoded showcase (linking to /marketplace),
   so the homepage never looks broken.
   Everything else (hero, beats, globe CTA, footer, routing) is byte
   identical to Batch 3. Backup already exists from Batch 3.
   ============================================================ */
const fs = require('fs');
...
if (!fs.existsSync(pagePath + '.v3bak')) fs.copyFileSync(pagePath, pagePath + '.v3bak');
fs.writeFileSync(pagePath, PAGE, 'utf8');
console.log('  +  app/page.tsx homepage grid now reads live DB and links each card to its PDP');
console.log('\n  Done. npm run build -> npm run dev -> click a homepage product -> lands on its product page. Then git push.\n');
