/* ============================================================
   batch17_typefix.js  —  Vendra v4, Batch 17 (build type fix)
   Drop in project root (C:\Users\kvngs\VENDRA), then run:
       node batch17_typefix.js
   ------------------------------------------------------------
   Fixes the Vercel type error in app/profile/page.tsx:
     "Type 'string | null' is not assignable to type
      'string | Blob | undefined'" on the order thumbnail <img>.
   The Thumb component now computes imgFor(o) into a local const
   so TypeScript narrows it after the && guard. Nothing else
   changes; order page is untouched (it already type-checks).
   ============================================================ */
const fs = require('fs');
const path = require('path');
const root = process.cwd();
const profilePath = path.join(root, 'app', 'profile', 'page.tsx');
if (!fs.existsSync(profilePath)) { console.error('\n  x  app/profile/page.tsx not found. Run from project root.\n'); process.exit(1); }
...
console.log('  +  app/profile/page.tsx fixed (Thumb narrows imgFor via local const)');
console.log('\n  Done. npm run build -> git push. Vercel should type-check clean now.\n');
