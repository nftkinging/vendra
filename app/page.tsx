...
const FEATURED = [
  { slug: 'nour-atelier', name: 'Gold Crescent Ring', seller: 'Nour Atelier', price: 45, cat: 'Fashion', glow: 'a', bg: 'linear-gradient(145deg,#1e1608 0%,#2d2010 40%,#3d2e14 100%)' },
  { slug: 'bytedrop', name: 'Dashboard UI Kit', seller: 'ByteDrop', price: 49, cat: 'Digital', glow: 's', bg: 'linear-gradient(145deg,#060d18 0%,#0d1828 40%,#142035 100%)' },
  { slug: 'solar-prints', name: 'Desert Bloom Print', seller: 'Solar Prints', price: 55, cat: 'Art', glow: 'a', bg: 'linear-gradient(145deg,#100810 0%,#1a0f20 40%,#261630 100%)' },
  { slug: 'soundvault', name: 'Lo-Fi Essentials', seller: 'SoundVault', price: 25, cat: 'Music', glow: 's', bg: 'linear-gradient(145deg,#040c08 0%,#0a1810 40%,#122018 100%)' },
];
const SELLERS = [
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <section className='v-hero'>
        <div className='v-atm'>
          <div className='v-orb-a' />
          <div className='v-orb-sl-l' />
          <div className='v-orb-sl-r' />
          <div className='v-grid-overlay' />
          <div className='v-hairline' />
        </div>
        <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 1000 }}>
          ...
          <div className='v-hero-eyebrow'>
            ...
            <span className='v-eyebrow-text' style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 10, fontWeight: 300, fontStyle: 'italic', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--a)' }}>Commerce Unchained</span>
            <div className='v-eyebrow-line' />
          </div>
          ...
          <div className='v-hero-actions'>
            <button className='btn-primary' onClick={handleGetStarted} disabled={ctaLoading}>
              {ctaLoading ? 'Loading...' : isConnected ? 'Open Your Store' : 'Get Started'}
            </button>
          ...
          <div className='v-showcase-card'>
            ...
            {[
              { icon: '💍', dot: 'a', name: 'Gold Crescent Ring — Nour Atelier', sub: 'Apr 12 · 0x4a7f...8f1a · Settled in 1.2s', amt: '+$45.00' },
              { icon: '🎵', dot: 's', name: 'Lo-Fi Essentials — SoundVault', sub: 'Apr 12 · 0x9f2a...cc33 · Settled in 0.9s', amt: '+$25.00' },
              { icon: '🎨', dot: 'a', name: 'Desert Bloom Print — Solar Prints', sub: 'Apr 11 · 0x7c3f...2e91 · Settled in 1.4s', amt: '+$55.00' },
            ].map(tx => (
              <div key={tx.name} className='v-sc-row'>
                <div className='v-sc-row-l'>
                  <div className={'v-sc-dot v-sc-dot-' + tx.dot}>{tx.icon}</div>
                  <div><div className='v-sc-tx-name'>{tx.name}</div><div className='v-sc-tx-sub'>{tx.sub}</div></div>
                </div>
      ...
      <div className='v-ticker-wrap'>
        <div className='v-ticker-track'>
          {[...TICKS, ...TICKS].map((t, i) => {
            const [a, b] = t.split('|');
            return <div key={i} className='v-ticker-item'><span>{a}</span><div className='v-ticker-dot' />{b}<div className='v-ticker-dot' /></div>;
          })}
        </div>
      </div>
      ...
      <div style={{ background: 'var(--bg2)', padding: '1px 0' }}>
        <div className='v-section'>
          ...
          <div className='v-section-head'>
            ...
            <Link href='/marketplace' className='v-view-all'>View all listings <span>→</span></Link>
          </div>
      ...
      <div style={{ background: 'var(--bg)' }}>
        <div className='v-section'>
          ...
          <div className='v-bento-grid'>
            <div className='v-bcard'>
              <div className='v-bcard-num'>01 — Settlement</div>
              <div className='v-bcard-title'>Paid in under two seconds</div>
              <div className='v-bcard-body'>Arc's Malachite consensus engine makes every transaction final in under two seconds. Sellers get paid the moment buyers confirm — not days later.</div>
              <div className='v-bcard-stat'>&lt;2s</div>
              <div className='v-bcard-stat-label'>Settlement Time · Arc Testnet</div>
            </div>
            <div className='v-bcard'>
              <div className='v-bcard-num'>02 — Fees</div>
              <div className='v-bcard-title'>Zero percent. Zero exceptions.</div>
              <div className='v-bcard-body'>Traditional platforms take 6–20% of every sale. Vendra takes nothing. Arc's low gas costs make this economically viable for the first time at scale.</div>
              <div className='v-bcard-stat'>0%</div>
              <div className='v-bcard-stat-label'>Platform Fee · Forever</div>
            </div>
            <div className='v-bcard v-bcard-wide'>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
                <div>
                  <div className='v-bcard-num'>03 — Comparison</div>
                  <div className='v-bcard-title'>What you keep vs. what they take</div>
                  <div className='v-bcard-body'>Every platform in this market takes a cut. eBay, Etsy, Poshmark — all charging sellers who have no real alternative. Until now.</div>
                </div>
      ...
      <div className='v-editorial'>
        <div className='v-editorial-bg' />
        <div className='v-editorial-vignette' />
        <div className='v-editorial-orb' />
        <div className='v-editorial-inner'>
          <div>
            ...
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 300, fontStyle: 'italic', color: 'var(--w35)', lineHeight: 1.8, marginBottom: 16, maxWidth: 400 }}>Every payment on Vendra is held in an ERC-8183 smart contract escrow until the buyer confirms delivery. No fake listings. No payment reversals. The blockchain is the guarantor.</p>
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 300, fontStyle: 'italic', color: 'var(--w35)', lineHeight: 1.8, marginBottom: 36, maxWidth: 400 }}>Add onchain identity via ERC-8004 — a permanent, unfakeable reputation score built from your entire transaction history. Real trust, not fake reviews.</p>
            <button className='btn-amber-ghost'>Learn About Security →</button>
          </div>
      ...
      <div style={{ background: 'var(--bg2)', padding: '1px 0' }}>
        <div className='v-section'>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64 }}>
            <div>
              <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 300, fontStyle: 'italic', color: 'var(--w35)', lineHeight: 1.8, maxWidth: 380, marginBottom: 32 }}>Every seller on Vendra owns their reputation onchain. It cannot be deleted, gamed, or transferred. What they build here, they keep forever.</p>
      ...
      <div style={{ background: 'var(--bg)', borderTop: '1px solid var(--b1)' }}>
        <div style={{ padding: '112px 56px', textAlign: 'center', maxWidth: 640, margin: '0 auto' }}>
          <div className='v-cta-rule'>
            <div className='v-cta-rule-line' />
            <div className='v-cta-rule-text'>Commerce Unchained</div>
            <div className='v-cta-rule-line' />
          </div>
          ...
          <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 300, fontStyle: 'italic', color: 'var(--w35)', lineHeight: 1.8, marginBottom: 44 }}>Zero fees. Instant settlement. Fraud-proof escrow. Borderless payments. Everything legacy platforms promised and never delivered.</p>
      ...
      <footer className='v-footer'>
        <div className='v-logo' style={{ opacity: 0.5 }}>
          <div className='v-logo-emblem' style={{ width: 28, height: 28 }}><span className='v-logo-v' style={{ fontSize: 12 }}>V</span></div>
          <span style={{ fontFamily: "'Cormorant',serif", fontSize: 16, fontWeight: 300, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--w85)' }}>Vendra</span>
        </div>
        ...
        <div className='v-footer-links'>
          ...
          <Link href='/store/create'>Sellers</Link>
