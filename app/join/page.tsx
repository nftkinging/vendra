...
export default function Join() {
  ...
  const [mode, setMode] = useState<'options'|'web3'|'circle'>('options');

  useEffect(() => { if (isConnected) router.push('/onboarding'); }, [isConnected, router]);

  // Check for existing Circle session
  useEffect(() => {
    try {
  ...
  const saveWallet = (w: {address:string;walletId:string;email:string}) => {
    setCircleWallet(w);
  ...
  const logout = () => {
    ...
    setMode('options'); setStep('email'); setEmail('');
  };
        ...
        {mode==='options' && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ border:'1px solid rgba(155,181,200,0.2)', padding:'24px', background:'rgba(155,181,200,0.03)', position:'relative', overflow:'hidden' }}>
              ...
              <div style={{ fontSize:12, fontWeight:300, fontStyle:'italic', color:'var(--w35)', marginBottom:16, lineHeight:1.6 }}>No crypto experience needed. Log in with your email.</div>
        ...
        {mode==='circle' && (
          <div style={{ border:'1px solid rgba(155,181,200,0.2)', background:'rgba(155,181,200,0.03)', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ padding:'24px' }}>
              ...
              {step==='notfound' && (
                <>
                  <div style={{ border:'1px solid var(--b1)', padding:'14px', marginBottom:20, background:'var(--bg3)' }}>
