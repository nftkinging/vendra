  ...
  if (!isSeller&&!isBuyer) return (
    <main style={{minHeight:'100vh',background:'var(--bg)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center'}}>
      ...
      <div style={{textAlign:'center'}}>
        <div style={{fontFamily:"'Cormorant',serif",fontSize:'clamp(36px,5vw,56px)',fontWeight:300,color:'var(--w)',marginBottom:16}}>No Profile Found</div>
        <div style={{fontSize:13,fontWeight:300,fontStyle:'italic',color:'var(--w35)',marginBottom:36,letterSpacing:'0.06em'}}>Complete onboarding to join Vendra</div>
      ...
      <div style={{paddingTop:72}}>
          ...
          <div style={{maxWidth:1200,margin:'0 auto'}}>
            <div style={{display:'flex',alignItems:'flex-start',gap:32,flexWrap:'wrap'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:"'Cormorant',serif",fontSize:'clamp(28px,4vw,48px)',fontWeight:300,color:'var(--w)',lineHeight:0.94,marginBottom:12}}>
                  {active?.display_name||'Vendra User'}
                  {view==='seller'&&store&&<span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:300,fontStyle:'italic',color:'var(--w18)',marginLeft:16,letterSpacing:'0.08em'}}>{'owner · '}{store.name}</span>}
                </div>
        ...
        {view==='seller'&&isSeller&&(
          <div style={{maxWidth:1200,margin:'0 auto',padding:'48px 56px 80px'}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'1px',background:'var(--b1)',marginBottom:48}}>
              {[{val:'$'+totalRevenue.toFixed(2),label:'Total Revenue',c:'var(--a2)'},{val:String(sellerOrders.length),label:'Orders',c:'var(--w)'},{val:String(store?.products?.length||0),label:'Products',c:'var(--w)'},{val:'0%',label:'Platform Fee',c:'var(--a2)'},{val:trustScore,label:'Trust Score · ERC-8004',c:'var(--gr)'}].map(s=>(
                <div key={s.label} className='v-profile-stat'><div className='v-profile-stat-val' style={{color:s.c}}>{s.val}</div><div className='v-profile-stat-label'>{s.label}</div></div>))}
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:24,marginBottom:48,alignItems:'start'}}>
              <div>
              ...
              <div style={{background:'linear-gradient(145deg,rgba(212,176,90,0.08),rgba(155,181,200,0.04),var(--bg2))',border:'1px solid rgba(212,176,90,0.15)',padding:24,position:'relative',overflow:'hidden'}}>
                ...
                <div style={{fontFamily:"'Cormorant',serif",fontSize:40,fontWeight:300,color:'var(--a2)',lineHeight:1,marginBottom:4}}>{'$'}{totalRevenue.toFixed(2)}</div>
                <div style={{fontSize:10,fontWeight:300,fontStyle:'italic',color:'var(--w18)',letterSpacing:'0.08em',marginBottom:20}}>USDC · Arc Testnet</div>
                <div style={{borderTop:'1px solid var(--b1)',paddingTop:16}}>
                  <div style={{fontSize:9,fontWeight:300,fontStyle:'italic',letterSpacing:'0.16em',textTransform:'uppercase',color:'var(--w18)',marginBottom:12}}>Recent Sales</div>
                  {sellerOrders.slice(0,3).length===0
                    ?<div style={{fontSize:11,fontWeight:300,fontStyle:'italic',color:'var(--w18)',textAlign:'center',padding:'12px 0'}}>No sales yet</div>
                    :sellerOrders.slice(0,3).map((o:any)=>(
                      <div key={o.id} style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
                        <div style={{width:32,height:32,background:'var(--a5)',border:'1px solid rgba(212,176,90,0.2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,flexShrink:0}}>{'📦'}</div>
                        <div style={{flex:1,minWidth:0}}><div style={{fontSize:11,fontWeight:300,color:'var(--w60)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{o.product_name}</div><div style={{fontSize:9,fontWeight:300,fontStyle:'italic',color:'var(--w18)'}}>{new Date(o.created_at).toLocaleDateString()}</div></div>
                        <div style={{fontFamily:"'Cormorant',serif",fontSize:13,fontWeight:300,color:'var(--gr)',flexShrink:0}}>{'+'}{Number(o.amount).toFixed(0)}</div>
                      </div>))}
                </div>
              </div>
            </div>
            ...
            <div style={{border:'1px solid var(--b1)'}}>
              {sellerOrders.length===0
                ?<div style={{padding:32,textAlign:'center',fontSize:12,fontWeight:300,fontStyle:'italic',color:'var(--w18)',letterSpacing:'0.10em'}}>No sales yet</div>
                :sellerOrders.map((o:any)=>(
                  <div key={o.id} style={{display:'flex',alignItems:'center',gap:16,padding:'16px 24px',borderBottom:'1px solid var(--b1)'}}>
                    <div style={{fontSize:'1.1rem'}}>{'📦'}</div>
                    <div style={{flex:1}}><div style={{fontFamily:"'Cormorant',serif",fontSize:16,fontWeight:300,color:'var(--w85)'}}>{o.product_name}</div><div style={{fontSize:10,fontWeight:300,fontStyle:'italic',color:'var(--w18)',marginTop:2}}>{new Date(o.created_at).toLocaleDateString()}{o.tx_hash?' · '+o.tx_hash.slice(0,14)+'...':''}</div></div>
                    <div style={{fontFamily:"'Cormorant',serif",fontSize:18,fontWeight:300,color:'var(--a2)'}}>{'$'}{Number(o.amount).toFixed(2)}</div>
                  </div>))}
            </div>
          </div>)}
        {/* BUYER VIEW */}
        {view==='buyer'&&isBuyer&&(
          <div style={{maxWidth:1200,margin:'0 auto',padding:'48px 56px 80px'}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',background:'var(--b1)',marginBottom:48}}>
              {[{val:'$'+totalSpent.toFixed(2),label:'Total Spent',c:'var(--sl)'},{val:String(buyerOrders.length),label:'Orders',c:'var(--w)'},{val:'Arc Testnet',label:'Network',c:'var(--w)'}].map(s=>(
                <div key={s.label} className='v-profile-stat'><div className='v-profile-stat-val' style={{color:s.c}}>{s.val}</div><div className='v-profile-stat-label'>{s.label}</div></div>))}
            </div>
            ...
            <div style={{border:'1px solid var(--b1)',marginBottom:32}}>
              {buyerOrders.length===0
                ?<div style={{padding:32,textAlign:'center',fontSize:12,fontWeight:300,fontStyle:'italic',color:'var(--w18)',letterSpacing:'0.10em'}}>No orders yet — <Link href='/marketplace' style={{color:'var(--a)'}}>explore the marketplace</Link></div>
                :buyerOrders.map((o:any)=>(
                  <Link key={o.id} href={'/orders/'+o.id} style={{textDecoration:'none',color:'inherit'}}>
                    <div style={{display:'flex',alignItems:'center',gap:16,padding:'16px 24px',borderBottom:'1px solid var(--b1)',cursor:'pointer',transition:'background 0.2s'}} onMouseEnter={e=>(e.currentTarget.style.background='var(--bg2)')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                      <div style={{fontSize:'1.1rem'}}>{'📦'}</div>
                      <div style={{flex:1}}><div style={{fontFamily:"'Cormorant',serif",fontSize:16,fontWeight:300,color:'var(--w85)'}}>{o.product_name}</div><div style={{fontSize:10,fontWeight:300,fontStyle:'italic',color:'var(--w18)',marginTop:2}}>{new Date(o.created_at).toLocaleDateString()}{' · click to view →'}</div></div>
                      <div style={{fontFamily:"'Cormorant',serif",fontSize:18,fontWeight:300,color:'var(--sl)'}}>{'$'}{Number(o.amount).toFixed(2)}</div>
                    </div>
                  </Link>))}
            </div>
