    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', background: 'radial-gradient(ellipse at 70% 50%, rgba(201,77,122,0.15), transparent 60%), radial-gradient(ellipse at 30% 50%, rgba(124,58,237,0.15), transparent 60%)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '8rem 2.5rem 3rem' }}>
          ...
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(2.5rem,6vw,4.5rem)', letterSpacing: '0.02em', lineHeight: 0.95, marginBottom: '1rem' }}>
            {store.name}
          </div>
          {store.tagline && (
            <div style={{ fontSize: '1rem', color: 'var(--muted)', fontWeight: 300, marginBottom: '0.75rem' }}>{store.tagline}</div>
          )}
          {store.description && (
            <div style={{ fontSize: '0.875rem', color: 'var(--muted)', fontWeight: 300, maxWidth: 500, lineHeight: 1.7 }}>{store.description}</div>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '3rem 2.5rem 4rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.6rem', color: 'var(--muted)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
