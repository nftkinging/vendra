...
const sampleStores: Record<string, any> = {
  'nour-atelier': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Gold Crescent Ring', price: 45, type: 'Physical', description: 'Sterling silver with gold plating', image_url: '' },
      { id: '2', name: 'Lapis Lazuli Earrings', price: 62, type: 'Physical', description: 'Genuine lapis lazuli stones', image_url: '' },
      { id: '3', name: 'Silver Khamsa Pendant', price: 38, type: 'Physical', description: 'Traditional Khamsa hand symbol', image_url: '' },
      { id: '4', name: 'Lookbook PDF', price: 8, type: 'Digital', description: 'Full season lookbook', image_url: '' },
    ],
  },
  'bytedrop': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Dashboard UI Kit', price: 49, type: 'Digital', description: 'Complete dashboard component library', image_url: '' },
      { id: '2', name: 'Icon Pack Pro', price: 19, type: 'Digital', description: '500+ custom icons', image_url: '' },
      { id: '3', name: 'Landing Page Templates', price: 35, type: 'Digital', description: '12 conversion-optimized templates', image_url: '' },
    ],
  },
  'solar-prints': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Desert Bloom Print', price: 55, type: 'Physical', description: 'A2 giclée print, limited to 50', image_url: '' },
      { id: '2', name: 'Neon City Poster', price: 40, type: 'Physical', description: 'A1 format, signed by artist', image_url: '' },
    ],
  },
  'kode-studio': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Landing Page Build', price: 299, type: 'Service', description: 'Custom Next.js landing page', image_url: '' },
  ...
  'umami-box': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Ramen Kit Deluxe', price: 28, type: 'Physical', description: 'Everything for authentic ramen', image_url: '' },
      { id: '2', name: 'Matcha Collection', price: 35, type: 'Physical', description: 'Ceremonial grade matcha set', image_url: '' },
    ],
  },
  'soundvault': {
    ...
    banner_url: '',
    products: [
      { id: '1', name: 'Lo-Fi Essentials Pack', price: 25, type: 'Digital', description: '50 lo-fi samples and loops', image_url: '' },
      { id: '2', name: 'Trap Drums Vol.2', price: 20, type: 'Digital', description: '200 one-shot drum samples', image_url: '' },
      { id: '3', name: 'Custom Beat License', price: 150, type: 'Digital', description: 'Exclusive rights to one beat', image_url: '' },
    ],
  },
};
...
export default function StorePage() {
  ...
  useEffect(() => {
    const load = async () => {
      if (sampleStores[slug]) {
        setStore(sampleStores[slug]);
        setLoading(false);
        return;
      }
  ...
  if (!store) return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.65rem', color: 'var(--muted)', marginBottom: '2rem', letterSpacing: '0.08em' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: store.banner_url ? '2rem 2.5rem 2.5rem' : '4rem 2.5rem 2.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1.5rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              {store.tagline && (
                <div style={{ fontSize: '0.95rem', color: 'var(--muted)', fontWeight: 300, marginBottom: '0.5rem' }}>
                  {store.tagline}
                </div>
              )}
              {xHandle && (
              ...
              {store.description && (
                <div style={{ fontSize: '0.875rem', color: 'var(--muted)', fontWeight: 300, maxWidth: 500, lineHeight: 1.7, marginTop: '0.5rem' }}>
                  {store.description}
                </div>
              )}
            </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '3rem 2.5rem 4rem' }}>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem', border: '1px solid var(--border)' }}>
            ...
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.65rem', color: 'var(--muted)', letterSpacing: '0.08em' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '1px', background: 'var(--border)' }}>
            {products.map((product: any) => (
              <div key={product.id} style={{ background: 'var(--bg)', transition: 'background 0.2s' }}
                ...
                {product.image_url ? (
                  ...
                  <div style={{ width: '100%', height: 160, background: 'radial-gradient(ellipse at 50% 50%, rgba(124,58,237,0.2), rgba(10,6,18,0.8))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem' }}>
                    📦
                  </div>
                )}
                ...
                <div style={{ padding: '1.25rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.3rem', letterSpacing: '0.04em', flex: 1, paddingRight: '1rem' }}>
                      {product.name}
                    </div>
                    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.55rem', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.2rem 0.5rem', border: '1px solid var(--border)', color: 'var(--muted)', flexShrink: 0 }}>
                      {product.type}
                    </div>
                  </div>
                  {product.description && (
                    <div style={{ fontSize: '0.85rem', color: 'var(--muted)', fontWeight: 300, lineHeight: 1.6, marginBottom: '1.25rem' }}>
                      {product.description}
                    </div>
                  )}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.8rem', color: 'var(--accent2)' }}>
                      ${product.price} USDC
                    </div>
