...
export default function AppKitWidget() {
  ...
  useEffect(() => {
    if (mounted && isConnected && address) fetchUsdcBalance();
  ...
  return (
    <div style={{ position:'relative' }}>
      <button onClick={() => { setOpen(!open); setError(''); setResult(''); if(!open) fetchUsdcBalance(); }} style={{ fontFamily:"'DM Sans',sans-serif", fontSize:11, fontWeight:400, background:'var(--bg2)', border:'1px solid var(--b2)', color:'var(--a2)', padding:'8px 14px', cursor:'pointer', display:'flex', alignItems:'center', gap:8, transition:'all 0.3s' }} onMouseEnter={e=>(e.currentTarget.style.borderColor='var(--a)')} onMouseLeave={e=>(e.currentTarget.style.borderColor='var(--b2)')}>
        ...
        <span style={{fontFamily:"'Cormorant',serif",fontSize:16,fontWeight:300}}>${usdcBalance}</span>
            ...
            <div style={{padding:16}}>
              {tab==='balance' && (
                <div style={{textAlign:'center'}}>
                  ...
                  <div style={{fontFamily:"'Cormorant',serif",fontSize:40,fontWeight:300,color:'var(--a2)',lineHeight:1,marginBottom:2}}>${usdcBalance}</div>
                ...
                <div>
                  ...
                  <div style={{border:'1px solid var(--b1)',background:'var(--bg3)',padding:'14px',marginBottom:14}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                      <div style={{fontSize:9,fontWeight:300,fontStyle:'italic',letterSpacing:'0.14em',textTransform:'uppercase',color:'var(--a)'}}>Arc Testnet Address</div>
                    </div>
              ...
              {tab==='bridge' && (
                <div>
                  <div style={{fontSize:11,fontWeight:300,fontStyle:'italic',color:'var(--w35)',lineHeight:1.7,marginBottom:12}}>Bridge USDC to Arc Testnet via Circle CCTP.</div>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:4,marginBottom:12}}>
                    {CHAINS.map(c=><button key={c.id} onClick={()=>setFromChain(c.id)} style={{padding:'8px 2px',textAlign:'center',background:fromChain===c.id?'rgba(212,176,90,0.12)':'var(--bg3)',border:fromChain===c.id?'1px solid var(--a)':'1px solid var(--b1)',cursor:'pointer',transition:'all 0.2s',display:'flex',flexDirection:'column',alignItems:'center',gap:3}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
                    ...
                    <span style={{color:'var(--a)',fontSize:'1.1rem'}}>→</span>
                    <div style={{flex:1,display:'flex',alignItems:'center',gap:6,border:'1px solid rgba(212,176,90,0.3)',padding:'8px 10px',background:'rgba(212,176,90,0.06)'}}>
                  ...
                  <button onClick={handleBridge} disabled={loading} className='btn-primary' style={{width:'100%',padding:'11px',fontSize:10}}>{loading?'Bridging...':`Bridge $${bridgeAmount} USDC → Arc`}</button>
                </div>)}
            </div>
          </div>
        </>
