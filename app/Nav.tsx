...
export default function Nav() {
  ...
  const [wallet, setWallet] = useState<{address:string;walletId:string}|null>(null);
  ...
  const closeAll = () => { setShowLogin(false); setShowCircle(false); setStep('email'); setError(''); setSendResult(''); };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.found) {
        setWallet({ address: data.address, walletId: data.walletId });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await fetchBalance(data.walletId);
      setStep('done');
    } catch(e:any) { setError(e.message || 'Failed to create wallet'); setStep('notfound'); }
  };
  ...
  const WalletDashboard = () => (
    <>
      <div style={{ border:'1px solid rgba(212,176,90,0.2)', background:'rgba(212,176,90,0.04)', padding:'20px', marginBottom:16, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ fontFamily:"'Cormorant',serif", fontSize:11, fontWeight:300, color:'var(--a)', wordBreak:'break-all', marginBottom:12 }}>{wallet?.address}</div>
      ...
      <div className='v-field'><label className='v-label'>Send to</label><input className='v-input' placeholder='0x...' value={sendForm.to} onChange={e => setSendForm({...sendForm, to:e.target.value})} /></div>
      ...
      <div style={{ display:'flex', gap:8 }}>
        ...
        <button onClick={closeAll} className='btn-slate' style={{ flex:1, fontSize:9, padding:'8px' }}>Close</button>
      </div>
    </>
  );
        ...
        <div className='v-modal-bg' onClick={closeAll}>
          <div className='v-modal' style={{ maxWidth:400 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ border:'1px solid rgba(155,181,200,0.2)', padding:'20px', background:'rgba(155,181,200,0.03)', position:'relative', overflow:'hidden' }}>
              ...
              <button onClick={() => setShowCircle(true)} className='btn-slate' style={{ width:'100%', fontSize:10, padding:'12px' }}>Log In with Circle Wallet →</button>
            </div>
          </div>
        </div>)}
      ...
      {showCircle && (
        <div className='v-modal-bg' onClick={closeAll}>
          <div className='v-modal' style={{ maxWidth:440 }} onClick={e => e.stopPropagation()}>
            ...
            {step==='email' && (
              <>
                <div style={{ fontSize:12, fontWeight:300, fontStyle:'italic', color:'var(--w35)', lineHeight:1.8, marginBottom:20 }}>Enter the email linked to your Circle wallet. If you don't have one yet, we'll create one for you.</div>
                ...
                <button onClick={() => setShowCircle(false)} style={{ width:'100%', background:'transparent', border:'none', fontSize:10, fontWeight:300, fontStyle:'italic', color:'var(--w18)', cursor:'pointer', padding:'8px' }}>← Back to options</button>
              </>
            )}
            ...
            {/* Step: Not found — offer to create */}
            {step==='notfound' && (
              <>
            ...
            {/* Step: Found or Done — show dashboard */}
            {(step==='found' || step==='done') && wallet && <WalletDashboard />}
          </div>
        </div>)}
