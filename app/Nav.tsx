...
export default function Nav() {
  ...
  const logout = () => { setCircleWallet(null); try { sessionStorage.removeItem(STORAGE_KEY); } catch {} setShowDashboard(false); closeAll(); };
    ...
    <>
      ...
      {showCircle && (
        <div className='v-modal-bg' onClick={closeAll}>
          <div className='v-modal' style={{maxWidth:440}} onClick={e=>e.stopPropagation()}>
            ...
            {step==='notfound'&&(<><div style={{border:'1px solid var(--b1)',padding:'16px',marginBottom:20,background:'var(--bg3)'}}><div style={{fontSize:11,fontWeight:300,fontStyle:'italic',color:'var(--w35)',lineHeight:1.7}}>No wallet found for <strong style={{color:'var(--w)'}}>{email}</strong>. Would you like to create one?</div></div>{error&&<div style={{fontSize:11,fontStyle:'italic',color:'var(--err)',marginBottom:12,border:'1px solid rgba(232,112,112,0.2)',padding:'8px 12px'}}>{error}</div>}<button onClick={handleCreate} className='btn-primary' style={{width:'100%',padding:'14px',fontSize:11,marginBottom:10}}>Create New Circle Wallet →</button><button onClick={()=>setStep('email')} style={{width:'100%',background:'transparent',border:'none',fontSize:10,fontWeight:300,fontStyle:'italic',color:'var(--w18)',cursor:'pointer',padding:'8px'}}>← Try a different email</button></>)}
      ...
      <nav className='v-nav'>
        ...
        <div className='v-nav-right'>
          ...
          <CartButton/>
          {/* Arc App Kit widget — shows for Web3 connected users */}
          {isConnected && <AppKitWidget />}
          {/* Auth buttons */}
          {isConnected
            ? <ConnectButton accountStatus='address' chainStatus='none' showBalance={false}/>
            : mounted && circleWallet
              ? <button onClick={()=>{ setShowDashboard(true); fetchBalance(circleWallet.walletId); }} className='btn-nav-amber' style={{display:'flex',alignItems:'center',gap:8,maxWidth:160}}>
                  <span>⭕</span>
                  <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{shortAddr(circleWallet.address)}</span>
                </button>
